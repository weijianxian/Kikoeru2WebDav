...
作品展开路径默认启用智能目录：`smart=1`、`ext=mp3`。这包括 `/RJxxxxxxx/`、`/01489611/`、`/popular/RJxxxxxxx/` 和 `/recommend/RJxxxxxxx/`。Worker 会用 `ext` 找到包含目标后缀文件的目录，然后返回这些目录的完整内容；例如某目录里有 `mp3` 和 `txt`，只要 `mp3` 命中，`txt` 也会一起出现在该目录里。
